...
/**
  ...
  \param ctx    Context of bind target words.
  \param it     First cell.
  \param end    End of cell array.
*/
...
as_word:
                ...
                if( wrdN > -1 )
                {
                    ...
                    // Hack to support BOR_BIND_OPTION_ARG.
                    // FIXME: sel[1] conflicts with 3rd path! node.
                    if( cell->word.binding == UR_BIND_USER+2 )
                        it->word.sel[1] = cell->word.sel[1];
                }
                break;
