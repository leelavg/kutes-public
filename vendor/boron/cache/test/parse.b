...
print "---- parse into recurse"
rnum: []
rrule: [some [tok: word! [into rrule | int! (append rnum second tok)]]]
print parse [
    rootA [childA 1 childB [grandA [item1 2] grandB 3]] rootB 4
] rrule
probe rnum


print "---- parse into fails"
prog: [render [shader sh1 varray ar1]]
rules: [some ['render into [some render-rules]]]
render-rules: ['shader word!]
print parse prog rules
render-rules: ['shader word! | 'varray word!]
print parse prog rules
...
probe try [
    ...
    more code   ; Never reached & should not be shown in trace.
]


print "---- multi-type"
probe parse [/transform orthogonal 320,200 /range 2 6] [
    any [seq: option! some int!/coord!/word! :seq (probe seq)]
]
