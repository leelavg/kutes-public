...
#define URLAN_H
/*
  Copyright 2009-2015,2019-2024 Karl Robillard
...
#define UR_VERSION_STR  "2.96.0"
#define UR_VERSION      0x0209600
...
enum UrlanDataType
{
    ...
    UT_VEC3,
    UT_TIMECODE,
    UT_WORD,    //--- The following values reference buffers.
    UT_LITWORD,
    UT_SETWORD,
    UT_GETWORD,
    UT_OPTION,
    UT_BINARY,  //--- Series values
    UT_BITSET,
    UT_STRING,
    UT_FILE,
    UT_VECTOR,
    UT_BLOCK,
    UT_PAREN,
    UT_PATH,    //--- Other non-series values
    UT_LITPATH,
    UT_SETPATH,
    UT_CONTEXT,
    UT_HASHMAP,
...
#define UR_SELECT_ATOM   1
#define UR_SELECT_INT    2
...
struct UThread
{
    ...
    UBuffer     holds;
    UBuffer     gcBits;
    UBuffer     blocksSeen;     // Prevent recursion in block functions.
    UCell       tmpWordCell;
...
typedef struct UEnvParameters UEnvParameters;

struct UEnvParameters
{
    const UDatatype** dtTable;  //!< Pointers to user defined datatypes.
    void (*envInit)(UEnv*, const UEnvParameters*);
    void (*threadMethod)(UThread*, enum UThreadMethod);
    uint16_t dtCount;           //!< Number of entries in dtTable.
    uint16_t atomLimit;         //!< Maximum number of atoms.
    uint32_t atomNamesSize;     //!< Fixed byte size of atom name buffer.
    uint32_t envSize;           //!< Byte size of environment structure.
    uint32_t threadSize;        //!< Byte size of thread structure.
    uint32_t user0;             //!< A user defined parameter.
};
...
#define ur_bufHeader(type,es,form,fl)   (type<<24 | es<<16 | form<<8 | fl)
#else
#define ur_setId(c,t)       *((uint32_t*) (c)) = t
#define ur_initCoord(c,n)   *((uint32_t*) (c)) = UT_COORD | (n) << 16
#define ur_bufHeader(type,es,form,fl)    (fl<<24 | form<<16 | es<<8 | type)
#endif
...
#define ur_isBlockType(t)   ((t) == UT_BLOCK || (t) == UT_PAREN)
#define ur_isPathType(t)    ((t) >= UT_PATH && (t) <= UT_SETPATH)
#define ur_isStringType(t)  ((t) == UT_STRING || (t) == UT_FILE)

#define ur_isSeriesType(t)  ((t) >= UT_BINARY && (t) < UT_PATH)
...
#define ur_bufferInit(b,head,count,data) \
    *((uint32_t*) b) = head; \
    b->used = count; \
    b->ptr.v = data
