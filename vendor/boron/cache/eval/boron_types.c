...
// Mark all blocks on the BoronFibre::evalOp stack.
static void eval_recycle(UThread* ut, int phase)
{
    if (phase == UR_RECYCLE_SWEEP) {
        const UBuffer* buf = &BT->evalOp;
        const EvalFrame* it = ur_ptr(EvalFrame, buf);
        const EvalFrame* end = it + buf->used;
        for (; it != end; ++it) {
            if (1<<it->block.eop & MASK_EOP_DO) {
                UIndex n = it->block.codeBlk;
                if (n > UR_INVALID_BUF) {
                    if (ur_markBuffer(ut, n))
                        block_markBuf(ut, ur_buffer(n));
                }
...
{
  {
    ...
    eval_recycle,           func_mark,              func_destroy,
