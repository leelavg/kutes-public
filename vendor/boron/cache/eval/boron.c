...
static void boron_threadInit( UThread* ut )
{
    ...
    ur_arrInit( &BT->frames, sizeof(UIndex), 16*2 );
    ur_arrInit( &BT->evalOp, sizeof(EvalFrame), env->evalStackSize );

    ur_arrReserve( &ut->stack, env->argStackSize );
    BT->stackLimit = env->argStackSize - 8;
    boron_reset( ut );
}
...
#ifdef DUMP_ARG_PROG
extern void boron_argProgramToStr(UThread*, const void* prog, UBuffer* str);
#endif
...
/**
  ...
  \li Must return UR_OK, UR_THROW or CFUNC_REFRAMED.
...
#ifdef DUMP_ARG_PROG
                str.used = 0;
                boron_argProgramToStr(ut, argProg->ptr.b + cell->argProgOffset,
                                      &str);
                ur_strTermNull(&str);
                printf("KR cfunc %s %s\n", ur_wordCStr(start), str.ptr.c);
#endif
            }
...
static void _addDatatypeWords( UThread* ut, int typeCount )
{
    ...
    for( i = 0; i < typeCount; ++i )
    {
        ...
        cp = bn_str_copy( cp, typeName );
        cp[-1] = '?';           // Change ! to ?.
        cp = bn_str_copy( cp, sig );

        cp = bn_str_copy( cp, "to-" );
        cp = bn_str_copy( cp, typeName );
        cp = bn_str_copy( cp - 1, sig );
...
UEnvParameters* boron_envParam( UEnvParameters* par )
{
    ...
    boron_setStackSize(par, 512, 192);
    return par;
}


static void boron_initStackSize(UEnv* uenv, const UEnvParameters* par)
{
    BoronEnv* env = (BoronEnv*) uenv;
    env->argStackSize  = par->user0 >> 16;
    env->evalStackSize = par->user0 & 0xffff;
}


#define USER0_ARG_EVAL(arg,eval) (((uint32_t) arg << 16) | eval)

void boron_setStackSize( UEnvParameters* par, uint16_t argStack,
                         uint16_t evalStack )
{
    par->user0 = USER0_ARG_EVAL(argStack, evalStack);
    par->envInit = boron_initStackSize;
}
...
#if 1
    {
    UCell* res = ur_stackTop(ut);
    if (ur_unserialize(ut, boot_data, boot_data + boot_len, res) &&
        ur_is(res, UT_BLOCK))
    {
        boron_bindDefault(ut, res->series.buf);
        if (! boron_evalBlock(ut, res->series.buf, ur_push(ut, UT_UNSET)))
            goto fail;
    }
    else
        goto fail;
...
#else
    // Use with "./mkboot -s"
    if( ! boron_evalUtf8( ut, boot_data, sizeof(boot_data)-1 ) )
        goto fail;
#endif
...
#define LOAD_STATIC_FILENAME
#ifdef LOAD_STATIC_FILENAME
    ur_bufferInit(str, ur_bufHeader(UT_STRING, 1, UR_ENC_UTF8, UR_STATIC),
                  strlen(file), (char*) file);
#else
