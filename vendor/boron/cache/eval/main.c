...
void usage( const char* arg0 )
{
    ...
    printf( "Options:\n"
            ...
            "  -f size  Set evaluation frame stack size (default is 192)\n"
            "  -g size  Set argument cell stack size (default is 512)\n"
        ...
        else if( cmd[0] != '\n' )
        {
            ...
            if( res )
            {
                ...
                if( rstr->ptr.c )
                {
                    ...
                    {
                        char* cp = bn_str_copy( rstr->ptr.c + PRINT_MAX - 4,
                                             "..." );
                        *cp = '\0';
                    }
                    printf( "== %s\n", rstr->ptr.c );
...
int argToInt(const char* arg, int minimum)
{
    int n = atoi(arg);
            ...
            for( ++arg; *arg != '\0'; ++arg )
            {
                switch( *arg )
                {
                    ...
                    case 'f':
                        if( ++i >= argc )
                            goto usage_err;
                        evalFrames = argToInt(argv[i], 128);
                        break;

                    case 'g':
                        if( ++i >= argc )
                            goto usage_err;
                        argStackCells = argToInt(argv[i], 256);
                        break;
...
usage_err:
    ...
    boron_setStackSize(boron_envParam(&param), argStackCells, evalFrames);
